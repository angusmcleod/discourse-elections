class DiscourseElections::ElectionController < ::ApplicationController
  ...
  def start_poll
    ...
    result = DiscourseElections::ElectionTopic.set_status(params[:topic_id], Topic.election_statuses[:poll])

    render_result(result || { error_message: I18n.t('election.errors.set_status_failed') })
  end
  ...
  def set_status
    ...
    result = DiscourseElections::ElectionTopic.set_status(params[:topic_id], params[:status].to_i)
  ...
  def set_nomination_message
    ...
    result = DiscourseElections::ElectionTopic.set_message(params[:topic_id], params[:nomination_message], 'nomination')

    render_result(result || { error_message: I18n.t('election.errors.set_message_failed')})
  end
  ...
  def set_poll_message
    ...
    result = DiscourseElections::ElectionTopic.set_message(params[:topic_id], params[:poll_message], 'poll')
  ...
  def set_position
    ...
    result = DiscourseElections::ElectionTopic.set_position(params[:topic_id], params[:position])

    render_result(result || { error_message: I18n.t('election.errors.set_position_failed')})
  end

  private

  def render_result(result = {})
    if result.try(:error_message)
      render json: failed_json.merge(message: result[:error_message])
    else
